...
$index = Array();
$url = "SELECT * FROM seeds";
$urlresult = mysqli_query($link,$url);
while($urlrows=mysqli_fetch_array($urlresult))
{
	$page[$i]=$urlrows['urls'];
	$i++;
}
...
{
$needlea = explode('/',$page[$j]);
$needle = $needlea[2];
$html = file_get_html("$page[$j]");
//$xml = new SimpleXMLElement("dataset.xml",0,TRUE);
...
foreach($html->find('a') as $element) 
       ...
       $t = $element->href; // Url of the page
	   ...
	   $index[$i] = $t;
	   $check = "SELECT urls FROM pages WHERE urls=\"$index[$i]\"";
	   $checkresult = mysqli_query($link,$check);
	   $checkrow = mysqli_fetch_array($checkresult);
	   if(count($checkrow)<=0)
	   {
	    $data = file_get_html("$index[$i]");
	   $plaindata = file_get_html("$index[$i]")->plaintext;
	   //echo $plaindata."<br>";
	   $titlep = $data->find('title','0');
	   $title = $titlep->plaintext;
	   $str = utf8_encode(file_get_html("$index[$i]")->plaintext);
	   $words = str_word_count(strtolower($str),1);
       $numWords = count($words);
	   $meta = "";
       $tags = get_meta_tags("$index[$i]");
	   if(!empty($tags['description'])){
	   $meta = $tags['description'];
	   }
// array_count_values() returns an array using the values of the input array as keys and their frequency in input as values.
       $word_count = (array_count_values($words));
	   $kw = array_keys($word_count);
	   $finalwords = array_diff($kw,$stopwords);
       arsort($finalwords);
	   $keywords = implode(",",$finalwords);
	   $pr = new PR();
	   $pagerank = $pr->get_google_pagerank($index[$i]);
	   $add = "INSERT INTO pages (urls,plaintext,title,keywords,metatag,pagerank) VALUES (\"$index[$i]\",\"$plaindata\",\"$title\",\"$keywords\",\"$meta\",\"$pagerank\")";
